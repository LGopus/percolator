...
```text
SUMMARY: 28 passed, 0 failed/timeout (0 timeout) out of 28
```
...
| Fail | 0 |
| Timeout | 0 |
| Total wall-clock harness time | 1240s |
| Slowest harness | `proof_v13_bankrupt_liquidation_cannot_free_exposure_before_residual_durable` |
| Slowest harness time | 407s |

## Harness Timings

| Harness | Time | Status |
|---|---:|---|
| `proof_v13_bankrupt_liquidation_cannot_free_exposure_before_residual_durable` | 407s | PASS |
| `proof_v13_account_b_chunk_either_advances_or_fails_closed` | 125s | PASS |
| `proof_v13_rebalance_reduce_position_preserves_senior_claims_and_reduces_risk` | 114s | PASS |
| `proof_v13_hlock_allows_pure_risk_reducing_trade_with_principal_margin` | 107s | PASS |
...
| `proof_v13_b_residual_booking_makes_durable_progress_or_fails_closed` | 46s | PASS |
...
| `proof_v13_target_effective_lag_rejects_risk_increasing_trade_before_mutation` | 11s | PASS |
| `proof_v13_hlock_rejects_risk_increasing_trade_before_mutation` | 10s | PASS |
| `proof_v13_side_reset_prior_epoch_account_can_clear_without_oi_underflow` | 10s | PASS |
...
| `proof_v13_fee_charge_settles_loss_before_fee` | 7s | PASS |
...
| File | Harnesses |
|---|---:|
| `tests/proofs_v13.rs` | 28 |

Strength indicators:

| Check | Result |
|---|---:|
| Harnesses over v13 production code | 28 |
| Harnesses with symbolic inputs or symbolic branch choices | 12 |
| Harnesses with `kani::cover!` reachability checks | 12 |
...
|---|---|
| Non-vacuity | No confirmed vacuous harnesses found. Cover checks exercise h-min/h-max, stale set/clear, hidden-leg rejection, B-chunk progress paths, bankrupt residual recovery, zero/partial insurance paths, resolved partial-B close progress, and both covered rebalance reduction paths. |
...
The Rust suite currently covers 50 wide-math unit tests and 41 v13 spec tests.
...
residual durability before exposure release, uncollectible liquidation-fee
exclusion from residual loss, resolved partial-B close liveness, resolved payout
snapshot fairness, rebalance strict risk-progress with senior-claim preservation,
and permissionless account-local progress. The retired v12 slab proofs no longer
apply after the architectural cutover.
