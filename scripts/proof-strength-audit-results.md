...
```text
SUMMARY: 27 passed, 0 failed/timeout (0 timeout) out of 27
```
...
| Fail | 0 |
| Timeout | 0 |
| Total wall-clock harness time | 1118s |
| Slowest harness | `proof_v13_bankrupt_liquidation_cannot_free_exposure_before_residual_durable` |
| Slowest harness time | 398s |

## Harness Timings

| Harness | Time | Status |
|---|---:|---|
| `proof_v13_bankrupt_liquidation_cannot_free_exposure_before_residual_durable` | 398s | PASS |
| `proof_v13_account_b_chunk_either_advances_or_fails_closed` | 126s | PASS |
...
| `proof_v13_bankrupt_liquidation_excludes_fee_from_residual_and_spends_insurance_once` | 71s | PASS |
...
| `proof_v13_b_residual_booking_makes_durable_progress_or_fails_closed` | 34s | PASS |
...
| File | Harnesses |
|---|---:|
| `tests/proofs_v13.rs` | 27 |

Strength indicators:

| Check | Result |
|---|---:|
| Harnesses over v13 production code | 27 |
| Harnesses with symbolic inputs or symbolic branch choices | 11 |
| Harnesses with `kani::cover!` reachability checks | 11 |
...
|---|---|
| Non-vacuity | No confirmed vacuous harnesses found. Cover checks exercise h-min/h-max, stale set/clear, hidden-leg rejection, B-chunk progress paths, bankrupt residual recovery, zero/partial insurance paths, and both covered rebalance reduction paths. |
...
The Rust suite currently covers 50 wide-math unit tests and 40 v13 spec tests.
...
exclusion from residual loss, resolved payout snapshot fairness, rebalance
strict risk-progress with senior-claim preservation, and permissionless
