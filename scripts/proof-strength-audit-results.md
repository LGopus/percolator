...
```text
SUMMARY: 26 passed, 0 failed/timeout (0 timeout) out of 26
```
...
| Fail | 0 |
| Timeout | 0 |
| Total wall-clock harness time | 1082s |
| Slowest harness | `proof_v13_bankrupt_liquidation_cannot_free_exposure_before_residual_durable` |
| Slowest harness time | 400s |

## Harness Timings

| Harness | Time | Status |
|---|---:|---|
| `proof_v13_bankrupt_liquidation_cannot_free_exposure_before_residual_durable` | 400s | PASS |
| `proof_v13_hlock_allows_pure_risk_reducing_trade_with_principal_margin` | 129s | PASS |
| `proof_v13_account_b_chunk_either_advances_or_fails_closed` | 125s | PASS |
| `proof_v13_rebalance_reduce_position_preserves_senior_claims_and_reduces_risk` | 117s | PASS |
| `proof_v13_bankrupt_liquidation_consumes_insurance_before_social_loss` | 60s | PASS |
| `proof_v13_b_residual_booking_makes_durable_progress_or_fails_closed` | 44s | PASS |
...
| `proof_v13_quantity_adl_preserves_oi_symmetry_after_close` | 14s | PASS |
...
| `proof_v13_full_refresh_clears_stale_certificate` | 4s | PASS |
...
| File | Harnesses |
|---|---:|
| `tests/proofs_v13.rs` | 26 |

Strength indicators:

| Check | Result |
|---|---:|
| Harnesses over v13 production code | 26 |
| Harnesses with symbolic inputs or symbolic branch choices | 10 |
| Harnesses with `kani::cover!` reachability checks | 10 |
...
|---|---|
| Non-vacuity | No confirmed vacuous harnesses found. Cover checks exercise h-min/h-max, stale set/clear, hidden-leg rejection, B-chunk progress paths, bankrupt residual recovery, and both covered rebalance reduction paths. |
...
The Rust suite currently covers 50 wide-math unit tests and 39 v13 spec tests.
...
blocking, bankrupt liquidation insurance-before-social-loss ordering, bankrupt
residual durability before exposure release, resolved payout snapshot fairness,
rebalance strict risk-progress with senior-claim preservation, and permissionless
account-local progress. The retired v12 slab proofs no longer apply after the
